...
export class GameHud {
    ...
    constructor(
        ...
        this.root.innerHTML = `<header class="race-header"><div class="wordmark">${COMPASS} PIRATE CUP</div><div class="race-title">THE TWIN ISLES <span>Local playtest</span></div><nav aria-label="Game options"><button class="quiet" data-action="sound" aria-label="${this.muted ? 'Unmute sound' : 'Mute sound'}">${this.muted ? 'Sound off' : 'Sound on'}</button><button class="quiet" data-action="pace" aria-label="${this.quick ? 'Use normal animations' : 'Enable quick animations'}">${this.quick ? '2× pace' : '1× pace'}</button><button class="quiet" data-action="export">Export save</button><button class="quiet" data-action="help">How to play</button><button class="quiet" data-action="menu">Harbor ↗</button></nav></header>
            ...
            <div class="course-legend"><span>Hover a hex for its coordinate · Numbers = safe corner speed</span><span><i class="legend-dot"></i> Your ship</span><span><i class="legend-dot amber"></i> Collision risk · Pale reefs = impassable</span></div>
            <section class="command-deck" aria-label="Captain’s controls"></section>
            <footer class="race-footer"><span class="round-status"></span><span class="save-status" role="status"></span><button data-action="log" class="log-button" aria-label="Open ship’s log"><span class="log-message" aria-live="polite"></span><span>Ship’s log ↗</span></button></footer>
    ...
    setSaveStatus(saved: boolean) {
        this.root.querySelector('.save-status')!.textContent = saved ? 'Saved in this browser' : 'Autosave unavailable — export to keep this race';
    }
        ...
        const content =
            kind === 'help'
                ...
                : kind === 'log'
                  ...
                  : `<p class="eyebrow gold">RETURN TO PORT</p><h2>Leave this race?</h2><p>Continue this race from the harbor. If browser saving is unavailable, export your save before leaving.</p><button class="primary" data-action="leave">Return to harbor →</button>`;
