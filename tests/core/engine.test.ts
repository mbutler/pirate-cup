...
describe('GameEngine', () => {
    ...
    it('starts movement immediately when the active player submits speed', () => {
        ...
        const result = reduce(
            createInitialState('test-seed', { playerCount: 2 }),
            {
        ...
        expect(result.events.some((e) => e.type === 'PHASE_CHANGED')).toBe(
            true,
        );
    });
    ...
    it('returns ram events when the final move of a turn is a ram', () => {
        ...
        expect(result.events.some((event) => event.type === 'RAMMING')).toBe(
            true,
        );
    ...
    it('auto-rolls mutiny speed as sails plus d10 on turn start', () => {
        ...
        expect(
            result.events.some((event) => event.type === 'MUTINY_SPEED_ROLLED'),
        ).toBe(true);
        expect(result.state.phase).toBe('movement');
        expect(result.state.ships['player-1'].chosenSpeed).toBeGreaterThan(
            baseShip.maxSpeed,
        );
        expect(result.state.ships['player-1'].movementRemaining).toBe(
            result.state.ships['player-1'].chosenSpeed,
        );
    ...
    it('ends the turn immediately when flogging draws mutiny', () => {
        ...
        expect(
            result.events.some((event) => event.type === 'MUTINY_STARTED'),
        ).toBe(true);
        expect(result.state.ships['player-1'].rowers.temperament).toBe(
            'mutiny',
        );
        expect(result.state.ships['player-1'].movementRemaining).toBe(0);
        expect(result.state.ships['player-1'].flogAttemptsRemaining).toBe(0);
    ...
    it('rejects flogging after the captain has passed control', () => {
        let state = createInitialState('flog-pass', { playerCount: 2 });
        const rng = createRng('flog-pass');
        state = reduce(
            state,
            {
                type: 'SUBMIT_TURN_INPUT',
